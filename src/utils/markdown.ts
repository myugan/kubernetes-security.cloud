...
export function renderSimpleMarkdown(markdown: string): string {
  ...
  let html = markdown
    ...
    .replace(
      /\[([^\]]+)\]\(([^)]+)\)/g,
      '<a href="$2" target="_blank" rel="noopener noreferrer" class="ds-link hover:underline">$1</a>'
    )
    ...
    .replace(
      /`([^`]+)`/g,
      '<code class="text-xs bg-app-code-bg border border-app-code-border text-app-code-text px-1.5 py-0.5 rounded font-mono">$1</code>'
    );
